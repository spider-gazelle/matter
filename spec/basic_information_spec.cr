...
describe Matter::Cluster::BasicInformationCluster do
  describe "initialization" do
    ...
    it "creates cluster with custom values" do
      ...
      cluster.vendor_id.should eq(0x1234_u16)
  ...
  describe "attributes" do
    it "has required attributes" do
      ...
      attrs.size.should be >= 15

      # Check for key attributes
      data_model = attrs.find { |a| a.id.id == Matter::Cluster::BasicInformationCluster::ATTR_DATA_MODEL_REVISION }
      ...
      vendor_name = attrs.find { |a| a.id.id == Matter::Cluster::BasicInformationCluster::ATTR_VENDOR_NAME }
      vendor_name.should_not be_nil
      vendor_name.not_nil!.type.should eq(:string)

      node_label = attrs.find { |a| a.id.id == Matter::Cluster::BasicInformationCluster::ATTR_NODE_LABEL }
    ...
    it "reads VendorName attribute" do
      ...
      # TODO: Proper TLV encoding not implemented yet
    ...
    it "reads Reachable attribute" do
      ...
      result = cluster.read_attribute(Matter::Cluster::BasicInformationCluster::ATTR_REACHABLE)
      result.should be_a(Bytes)
      result.as(Bytes).should eq(Bytes[1])
    end

    it "reads SerialNumber attribute" do
      endpoint = Matter::DataType::EndpointNumber.new(0_u16)
      cluster = Matter::Cluster::BasicInformationCluster.new(
        endpoint,
      ...
      result = cluster.read_attribute(Matter::Cluster::BasicInformationCluster::ATTR_SERIAL_NUMBER)
      result.should be_a(Bytes)
      # TODO: Proper TLV encoding not implemented yet
      result.as(Bytes).size.should eq(0)
    end
    ...
    it "writes NodeLabel attribute" do
      ...
      # Note: TLV decoding not implemented yet, so node_label won't actually update
    ...
    it "writes Location attribute" do
      ...
      status.as(Matter::InteractionModel::Status).success?.should be_true
      # Note: TLV decoding not implemented yet, so location won't actually update
    end
