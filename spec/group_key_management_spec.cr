require "./spec_helper"
require "../src/matter/clusters/group_key_management"
  ...
  describe GroupKeyManagement do
    describe "initialization" do
      it "creates cluster with default settings" do
        ...
        cluster.features.should eq(GroupKeyManagement::Feature::None)
      end
    ...
    describe "GroupKeySetStruct validation" do
      it "accepts valid key set with single epoch key" do
        key_set = Clusters.create_key_set(id: 1_u16, key0: Clusters.test_key, time0: 1000_u64)
        key_set.validate!
      end

      it "accepts valid key set with multiple epoch keys in order" do
        key_set = Clusters.create_key_set(
          id: 1_u16,
          key0: Clusters.test_key(0x01), time0: 1000_u64,
          key1: Clusters.test_key(0x02), time1: 2000_u64,
          key2: Clusters.test_key(0x03), time2: 3000_u64
        )
        key_set.validate!
      end
      ...
      it "rejects unordered epoch start times" do
        key_set = Clusters.create_key_set(
          id: 1_u16,
          key0: Clusters.test_key(0x01), time0: 3000_u64, # Out of order
          key1: Clusters.test_key(0x02), time1: 2000_u64,
          key2: Clusters.test_key(0x03), time2: 1000_u64
        )
      ...
      it "rejects equal epoch start times" do
        key_set = Clusters.create_key_set(
          id: 1_u16,
          key0: Clusters.test_key(0x01), time0: 1000_u64,
          key1: Clusters.test_key(0x02), time1: 1000_u64 # Same as time0
        )
        expect_raises(ArgumentError, /must be strictly ordered/) do
          key_set.validate!
        end
    ...
    describe "KeySetWrite command" do
      it "creates a new key set" do
        cluster = GroupKeyManagement.new
        key_set = Clusters.create_key_set(id: 1_u16)
      ...
      it "updates an existing key set" do
        ...
        key_set1 = Clusters.create_key_set(id: 1_u16, key0: Clusters.test_key(0x01), time0: 1000_u64)
        ...
        # Update with new key
        key_set2 = Clusters.create_key_set(id: 1_u16, key0: Clusters.test_key(0x02), time0: 2000_u64)
        cluster.handle_key_set_write(
      ...
      it "enforces max_group_keys_per_fabric limit" do
        cluster = GroupKeyManagement.new(max_group_keys_per_fabric: 2_u16)
        fabric_index = 1_u8

        # Create two key sets (should succeed)
        cluster.handle_key_set_write(
          GroupKeyManagement::KeySetWriteRequest.new(Clusters.create_key_set(id: 1_u16)),
          fabric_index
        )
        cluster.handle_key_set_write(
          GroupKeyManagement::KeySetWriteRequest.new(Clusters.create_key_set(id: 2_u16)),
          fabric_index
        )
        ...
        expect_raises(ArgumentError, /Cannot exceed max_group_keys_per_fabric/) do
          cluster.handle_key_set_write(
            GroupKeyManagement::KeySetWriteRequest.new(Clusters.create_key_set(id: 3_u16)),
            fabric_index
          )
        end
      end
      ...
      it "allows updating without counting against limit" do
        ...
        cluster.handle_key_set_write(
          GroupKeyManagement::KeySetWriteRequest.new(
            Clusters.create_key_set(id: 1_u16, key0: Clusters.test_key(0xFF), time0: 5000_u64)
          ),
          fabric_index
        )
      end
      ...
      it "creates IPK key set (ID 0)" do
        cluster = GroupKeyManagement.new
        ipk_key_set = Clusters.create_key_set(id: 0_u16)
    ...
    describe "KeySetRead command" do
      it "reads existing key set" do
        cluster = GroupKeyManagement.new
        fabric_index = 1_u8
        key_set = Clusters.create_key_set(id: 1_u16, key0: Clusters.test_key(0xAA), time0: 1234_u64)
      ...
      it "sanitizes key material in response" do
        ...
        key_set = Clusters.create_key_set(
          id: 1_u16,
          key0: Clusters.test_key(0xAA), time0: 1000_u64,
          key1: Clusters.test_key(0xBB), time1: 2000_u64
        )
    ...
    describe "KeySetRemove command" do
      ...
      it "protects IPK (key set 0) from removal" do
        ...
        # Create IPK
        cluster.handle_key_set_write(
          GroupKeyManagement::KeySetWriteRequest.new(Clusters.create_key_set(id: 0_u16)),
          fabric_index
        )
    ...
    describe "KeySetReadAllIndices command" do
      ...
      it "returns all key set IDs for fabric" do
        cluster = GroupKeyManagement.new(max_group_keys_per_fabric: 10_u16)
        fabric_index = 1_u8

        # Create multiple key sets
        [0_u16, 1_u16, 5_u16, 10_u16].each do |id|
          cluster.handle_key_set_write(
            GroupKeyManagement::KeySetWriteRequest.new(Clusters.create_key_set(id: id)),
            fabric_index
          )
        end
    ...
    describe "group key map operations" do
      ...
      it "enforces max_groups_per_fabric limit" do
        cluster = GroupKeyManagement.new(max_groups_per_fabric: 2_u16)
        fabric_index = 1_u8
    ...
    describe "helper methods" do
      ...
      it "retrieves key set with actual key material" do
        cluster = GroupKeyManagement.new
        fabric_index = 1_u8
        key_bytes = Clusters.test_key(0xAA)
        key_set = Clusters.create_key_set(id: 1_u16, key0: key_bytes, time0: 1000_u64)
